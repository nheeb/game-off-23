...
func fall_down(target_pos: Vector3):
	...
	tween.tween_property(self, "global_position", Functions.get_nearest_ground(target_pos), 1.0).from(target_pos + Vector3.UP * 20.0).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
