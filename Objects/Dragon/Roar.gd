...
func effect_start(index):
	...
	if dragon.stage == 3:
		dragon.air_knockback_area.activate(.5)
	await get_tree().create_timer(.8).timeout
	for i in range(randi_range(3, 5) + dragon.stage):
		...
		var target_pos = dragon.global_position + Vector3(randf_range(-1, 1) * 13.0, 5.0, randf_range(-1, 1) * 13.0)
		if i == 2 and dragon.stage == 3:
			target_pos = Game.player.global_position + Vector3.UP * 5
		boulder.fall_down(target_pos)
		if (i % 5 == 0):
